...
def read_imageset(imset_dir, create_patches=False, patch_size=64, seed=None, top_k=None, beta=0.):
    ...
    if exists(join(imset_dir, 'HR.png')):
        hr = np.array(io.imread(join(imset_dir, 'HR.png')))#, dtype=np.uint8)
        hrcv = cv2.imread(join(imset_dir, 'HR.png'))
        print("HR After loading: ", hr)
        print(" HR with CV: ", hrcv)
    else:
